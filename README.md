...
| `inspect` | Inspect system objects, processes, or resources in detail. | `--process <pid>` (inspect process)<br>`--file <path>` (inspect file)<br>`--env <key>` (inspect env var)<br>`--system` (system info)<br>`--json`<br>`--raw` (unformatted output) |
| `calc` | Evaluate mathematical expressions (built-in calculator). | `--expr <expression>` (evaluate expression)<br>`--precision <n>` (decimal precision)<br>`--base <2/10/16>` (number base)<br>`--json` |
| `echo` | Print and transform text or system information. | `--text <msg>`<br>`--env <key>`<br>`--json`<br>`--color <style>`<br>`--encode <type>`<br>`--decode <type>`<br>`--speak` |
